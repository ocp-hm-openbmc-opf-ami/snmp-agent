        ...
        switch (userEvent)
        {
            ...
            case UserUpdateEvent::userRenamed:
            {
                ...
                retVal = snprintf(
                    ...
                    sizeof(userData->user[usrIndex].userName), "%s",
                    newUserName.c_str());
                if ((retVal < 0) ||
                    (retVal >=
                     (signed int)sizeof(userData->user[usrIndex].userName)))
                {
                    lg2::error("Unhandled user event: {USER_EVENT}",
...
Cc UserAccess::setUserName(const uint8_t userId, const std::string& userName)
{
    ...
    else if (oldUser.empty() && !userName.empty() && validUser)
    {
        try
        {
            ...
            // find snmp group and remove it, by default user privilege user
            // should not have snmp privilege
...
void UserAccess::readUserData()
{
    ...
    for (size_t usrIndex = 1; usrIndex <= ipmiMaxUsers; ++usrIndex)
    {
        ...
        retVal = snprintf(
            reinterpret_cast<char*>(usersTbl.user[usrIndex].userName),
            sizeof(usersTbl.user[usrIndex].userName), "%s", userName.c_str());
        if ((retVal < 0) ||
            (retVal >= (signed int)sizeof(usersTbl.user[usrIndex].userName)))
        {
            throw std::runtime_error(
                "Corrupted IPMI user data file - invalid length of user name ");
        }
                              ...
                              const std::string& sysPriv, const bool& enabled)
{
    ...
    retVal = snprintf(
        reinterpret_cast<char*>(userData->user[freeIndex].userName),
        sizeof(userData->user[freeIndex].userName), "%s", userName.c_str());
    if ((retVal < 0) ||
        (retVal >= (signed int)sizeof(userData->user[freeIndex].userName)))
    {
        lg2::error("Ivalid user name length");
        return false;
    }
    uint8_t priv =
